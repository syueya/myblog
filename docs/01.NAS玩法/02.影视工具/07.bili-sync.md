...
如果是第一次部署时没记录下来，后面又重新部署容器把日志刷掉了，可以停掉 bili-sync 后用 DB Browser for SQLite 或类似的工具打开 data.sqlite，config 表里存储了 json 格式的配置信息。如果config表里面没有信息的话可以试试删除这个config表重新启动容器。
...
点击设置-基本设置-后端API认证Token，这个值默认是我们刚刚在日志里的token，我们可改成自己的token值，然后点击保存。再去点击上部前端认证的退出认证，用我们更改的token来重新认证。

操作完成后可以重启容器，我们再去数据库中查看，config表里token的值会变成我们刚刚填写的token。

​![img](./img/bili-15.png)

## B 站认证

> 注意：V2.10.0 开始，B站支持扫码认证
